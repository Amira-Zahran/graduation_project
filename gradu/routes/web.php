...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware('admin')->prefix('Admin')->group(function () {
            ...
            Route::prefix('Employees')->group(function () {
                Route::get('list', [UserController::class, 'getAllUsers'])->name('employees-lists');
                Route::any('changeStatus', [UserController::class, 'changeStatus'])->name('changeStatus');
                Route::view('create', 'apps.employees.forms-new-employee')->name('employee-create');
                Route::post('store', [UserController::class, 'store'])->name('employee-store');
                Route::get('/edit/{id}', [UserController::class, 'show'])->name('employee-edit');
                Route::any('update', [UserController::class, 'update'])->name('employee-update');
            });

            Route::prefix('Customers')->group(function (){
                Route::get('list', [CustomerController::class, 'getAllCustomerData'])->name('customers-lists');
                Route::get('delete', [CustomerController::class, 'delete'])->name('customer-delete');
                Route::post('new',function (Illuminate\Http\Request $request){
                    $data = (new CustomerController())->newCustomer($request);
                    if($data->status() == 200){
                        return redirect()->route('customers-lists')->with('success',"Customer created successfully with id ".$data->original['id']);
                    }
                    return redirect()->route('customers-lists')->with('error',$data->original['message']);
                })->name('customer-new');
            });

            Route::prefix('Menu')->group(function (){
                Route::get('listCategory', function (){
                    $categories = (new \App\Http\Controllers\MenuController())->MenuData(request())->original['categories'];
                    return view('apps.menu.lists',compact('categories'));
                })->name('menu-categories-lists');
                Route::get('listItems', function (Illuminate\Http\Request $request){
                    $items = (new \App\Http\Controllers\MenuController())->MenuData(request())->original;
...
//                    $items->appends($request->all());
                    return view('apps.menu.products',compact('items'));
                })->name('menu-items-list');
                Route::any('dropCategory',[\App\Http\Controllers\MenuController::class, 'dropCategory'])->name('dropCategory');
                Route::post('updateCategory',[\App\Http\Controllers\MenuController::class, 'updateCategory'])->name('menu-category-update');
                Route::post('addCategory',[\App\Http\Controllers\MenuController::class, 'addCategory'])->name('menu-category-create');
                Route::post('updateItem',[\App\Http\Controllers\MenuController::class, 'updateItem'])->name('menu-item-update');
                Route::post('createItem',[\App\Http\Controllers\MenuController::class, 'createItem'])->name('menu-item-create');
            });

            Route::prefix('Orders')->group(function (){
                Route::any('createPDF', [PrintingController::class, 'createOrderReceiptPDF'])->name('createPDF');
                Route::get('liveapi', [\App\Http\Controllers\OrderController::class, 'liveapi'])->name('liveapi');
                Route::get('live', function (){
                    return view('apps.orders.liveOrders');
                })->name('orders-live');
                Route::get('list', [\App\Http\Controllers\OrderController::class, 'index'])->name('orders-lists');
                Route::get('/edit/{id}', [\App\Http\Controllers\OrderController::class, 'show'])->name('order-edit');
            });


            /* Create Shift Route*/
            Route::prefix('Shifts')->group(function (){
                Route::get('list', [\App\Http\Controllers\ShiftController::class, 'index'])->name('shifts-lists');
                Route::get('history', [\App\Http\Controllers\ShiftController::class, 'history'])->name('shift-history');
                Route::post('/update', [\App\Http\Controllers\ShiftController::class, 'update'])->name('shift-update');
                Route::post('openShift', [\App\Http\Controllers\ShiftController::class, 'openShift'])->name('shift-open');
                Route::get('closeShift', [\App\Http\Controllers\ShiftController::class, 'closeShift'])->name('shift-close');
                Route::get('report', [\App\Http\Controllers\ShiftController::class, 'shiftReportView'])->name('shift-report');
                Route::get('reportPDF', [\App\Http\Controllers\PrintingController::class, 'createShiftReportPDF'])->name('shift-report-pdf');
            });

            Route::prefix('DeliveryAgents')->group(function (){
                Route::get('list', [\App\Http\Controllers\DeliveryAgentsController::class, 'index'])->name('delivery-agents-lists');
                Route::get('suspend', [\App\Http\Controllers\DeliveryAgentsController::class, 'suspend'])->name('delivery-agents-suspend');
                Route::get('activate', [\App\Http\Controllers\DeliveryAgentsController::class, 'activate'])->name('delivery-agents-activate');
                Route::post('new', [\App\Http\Controllers\DeliveryAgentsController::class, 'newAgent'])->name('delivery-agents-new-agent');
                Route::post('updateAgent', [\App\Http\Controllers\DeliveryAgentsController::class, 'updateAgent'])->name('delivery-agents-update-agent');
                Route::get('history', [\App\Http\Controllers\DeliveryAgentsController::class, 'shiftsHistory'])->name('delivery-agents-shifts-history');
            });
