...
<div id="invoice-POS">
    ...
    <div id="mid">
        <div class="info">
            ...
            <p>sales receipt :    {{$order['num']}}</p>
            <p class="shadow ">bill num. : {{$order['bill']}}</p>
            <p>type : {{$order['type']}}</p>
            <p class="shadow">performed on : {{$order['date']}}</p>
            <p>cashier : {{$order['agent']}}</p>
        </div>
    </div><!--End Invoice Mid-->

    @if(str_contains($order['en_type'],'Delivery'))
        <div id="mid">
            <div class="info">
                <h2>Customer Info</h2>
                {{--            <p>--}}
                <p class="shadow ">name :    {{$order['customer']['name'] }}</p>
                <p >comments :    {{$order['customer']['comments']}}</p>
                @foreach($order['customer']['phones'] as $phone)
                    <p>Phone Number : {{$phone->phone_number}}</p>
                @endforeach
                <p class="shadow">address : {{$order['customer']['address']}}</p>
            </div>
        </div>
    @endif
    <div>
        ...
        <div id="table">
            <table>
                ...
                <tr class="tabletitle" dir="ltr">
                    ...
                    <td class="payment" dir="ltr"><h2>{{$order['total']}}</h2></td>
                </tr>
